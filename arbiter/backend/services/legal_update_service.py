...
Fetches new laws, court judgments, regulatory changes for all supported countries.
Stores results in Firestore `legal_updates` collection.
Called weekly via POST /api/admin/legal-updates/refresh (Cloud Scheduler).
...
Update structure:
  {
    ...
    "title": "...",
    "summary": "...",
    "impact": "How this affects ordinary people",
    "source_url": "...",   # from Gemini grounding
    "effective_date": "...",
    "fetched_at": "...",
    "week_number": 21,      # ISO week number
    "year": 2026,
  }
"""
...
- Avoid purely procedural/administrative court orders that don't affect citizen rights
...
async def fetch_legal_updates_for_country(
    ...
    """
    Use Gemini + grounding to fetch recent legal updates for one country.

    Returns a list of update dicts ready to store in Firestore.
    """
    import json, re
    ...
    try:
        # Use grounding for real-time search results
        result = await gemini_service.generate_with_grounding(prompt)

        # Extract JSON from response
        json_match = re.search(r'\[.*\]', result, re.DOTALL)
        if not json_match:
            logger.warning("legal_update_no_json", country=country_code)
            return []
        ...
        logger.info("legal_updates_fetched", country=country_code, count=len(processed))
        return processed
...
class LegalUpdateService:
    """Manages legal update fetching and storage."""
    ...
    async def refresh_all_countries(self) -> dict[str, int]:
        """
        Fetch and store updates for all supported countries.
        Returns {country_code: update_count}.
        """
        ...
        for code, data in SUPPORTED_COUNTRIES.items():
            ...
            logger.info("legal_updates_stored", country=code, count=len(updates))

        return results

    async def refresh_country(self, country_code: str) -> int:
        """Refresh updates for a single country."""
        from core.countries import SUPPORTED_COUNTRIES
    ...
    def get_updates(
        ...
        """Get stored legal updates with optional filters."""
        try:
            cutoff = (datetime.now(timezone.utc) - timedelta(days=days_back)).isoformat()
            query = self._fb.db.collection("legal_updates").where(
                "fetched_at", ">=", cutoff
            )
            if country_code:
